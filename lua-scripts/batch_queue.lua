...
local function schedule_process(self, batch, delay)
  ...
  self.process_scheduled_count = self.process_scheduled_count + 1
end
...
function Queue.new(process, opts)
  ...
  assert(opts.max_concurrent_processes == nil or type(opts.max_concurrent_processes) == "number",
         "max_concurrent_processes must be a number")
  ...
  local self = {
    ...
    max_concurrent_processes = opts.max_concurrent_processes or 100,
...
function Queue:flush()
  ...
  -- the queue is empty
  if #self.batch_queue > 0 and self.process_scheduled_count < self.max_concurrent_processes  then
